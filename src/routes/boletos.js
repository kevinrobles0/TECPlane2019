    ...
    if(errores.length>0){
        ...
        await vuelos.findOne({idVuelo:vuelo},async(err,vueloComprar)=>{
            ...
            if(!vueloComprar){
                ...
                var disponibles = vueloComprar.disponibles;

                if(parseInt(disponibles)>= parseInt(boletos)){
                    var nuevaDisponibles = parseInt(disponibles)-parseInt(boletos);
                    ...
                    if(encontro==false){
                        ...
                        infoCompra.push([vuelo,boletos,maletas,observaciones]);
                        await cliente.updateOne({idPasajero:idCliente},{$set:{infoCompra:infoCompra}},function(err,resp){
                            if(err){
                                console.log(err);
                            }else{
                                console.log(resp);
                        ...
                        await vuelos.updateOne({idVuelo:vuelo},{$set:{boletos:boletosEncontrados,disponibles:nuevaDisponibles}},function(err,resp){
                            if(err){
                                console.log(err);
                            }else{
                                console.log(resp);
                ...
                errores.push({text:"No hay suficientes boletos disponibles"});
                res.render("cliente/boletosSeleccionar",{errores});
                }
            }
        })
